...
router.get('/', validateConcursoFilters, async (req: Request, res: Response) => {
  try {
    logger.info('Início da requisição GET /api/concursos', { component: 'backend', query: req.query });
    ...
    logger.error('Erro inesperado no endpoint GET /api/concursos', { component: 'backend', error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
    logger.error('Erro ao processar requisição GET /api/concursos:', error);
    res.status(500).json({
      success: false,
...
router.get('/:id', validateConcursoId, async (req, res) => {
  try {
    ...
    if (error) {
      ...
      logger.error('Erro ao buscar concurso', { error: error.message });
      res.status(500).json({
        success: false,
    ...
    logger.error('Erro ao processar requisição GET /api/concursos/:id', { error: error instanceof Error ? error.message : String(error) });
    res.status(500).json({
      success: false,
      error: 'Erro interno no servidor',
    });
...
router.post('/', requireAuth, validateCreateConcurso, async (req, res) => {
  try {
    ...
    const { data: concurso, error } = await supabase
      ...
      .single();
    if (error) {
      logger.error('Erro ao criar concurso', { error: error.message });
      if (error.code === '23505') {
        res.status(400).json({
          success: false,
    ...
    logger.error('Erro ao processar requisição POST /api/concursos', { error: error instanceof Error ? error.message : String(error) });
    res.status(500).json({
      success: false,
      error: 'Erro interno no servidor',
    });
...
router.put('/:id', requireAuth, validateConcursoId, validateUpdateConcurso, async (req, res) => {
  try {
    ...
    if (error) {
      ...
      logger.error('Erro ao atualizar concurso', { error: error.message });
      res.status(500).json({
        success: false,
    ...
    logger.error('Erro ao processar requisição PUT /api/concursos/:id:', error);
    res.status(500).json({
      success: false,
      error: 'Erro interno no servidor',
    });
...
router.delete('/:id', requireAuth, validateConcursoId, async (req, res) => {
  try {
    ...
    const { error } = await supabase
      ...
      .eq('id', id);
    if (error) {
      logger.error('Erro ao deletar concurso', { error: error.message });
      res.status(500).json({
        success: false,
    ...
    logger.error('Erro ao processar requisição DELETE /api/concursos/:id:', error);
    res.status(500).json({
      success: false,
      error: 'Erro interno no servidor',
    });
...
router.patch('/:id/activate', requireAuth, validateConcursoId, async (req, res) => {
  try {
    ...
    if (error) {
      ...
      logger.error('Erro ao ativar/desativar concurso', { error: error.message });
      res.status(500).json({
        success: false,
    ...
    logger.error('Erro ao processar requisição PATCH /api/concursos/:id/activate:', error);
    res.status(500).json({
      success: false,
      error: 'Erro interno no servidor',
    });
