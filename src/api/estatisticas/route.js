    ...
    try {
      ...
      `)
        ...
        flashcardsDisciplinas?.forEach(item => {
            const disciplina = item.flashcards?.disciplina || 'Sem disciplina';
        ...
        simuladosDisciplinas?.forEach(item => {
            const disciplinasSimulado = item.simulados?.simulado_questoes?.map((q) => q.disciplina) || [];
        ...
        questoesDisciplinas?.forEach(item => {
            const disciplina = item.questoes_semanais?.disciplina || 'Sem disciplina';
            if (!disciplinas[disciplina]) {
                disciplinas[disciplina] = {
                    disciplina,
                    flashcards: { acertos: 0, erros: 0, tempo: 0 },
