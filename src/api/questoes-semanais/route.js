    ...
    try {
      ...
      `)
        ...
        if (error) {
            ...
            logger.error('Erro ao buscar questão semanal:', { error: error.message });
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
...
router.post('/', requireAuth, validateRequest(createQuestaoSemanalSchema), async (req, res) => {
    try {
        ...
        const { data: questao, error } = await supabase
            ...
            .single();
        if (error) {
            logger.error('Erro ao criar questão semanal:', { error: error.message });
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
        ...
        logger.error('Erro na rota POST /questoes-semanais:', { error: error instanceof Error ? error.message : 'Erro desconhecido' });
        res.status(500).json({ error: 'Erro interno do servidor' });
    }
});
...
router.put('/:id', requireAuth, validateRequest(updateQuestaoSemanalSchema), async (req, res) => {
    try {
        ...
        if (error) {
            ...
            logger.error('Erro ao atualizar questão semanal:', { error: error.message });
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
        res.json({ success: true, data: questao });
        ...
        logger.error('Erro na rota PUT /questoes-semanais/:id:', { error: error instanceof Error ? error.message : 'Erro desconhecido' });
        res.status(500).json({ error: 'Erro interno do servidor' });
    }
});
...
router.delete('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const { error } = await supabase
            ...
            .eq('id', id);
        if (error) {
            logger.error('Erro ao deletar questão semanal:', { error: error.message });
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
        ...
        logger.error('Erro na rota DELETE /questoes-semanais/:id:', { error: error instanceof Error ? error.message : 'Erro desconhecido' });
        res.status(500).json({ error: 'Erro interno do servidor' });
    }
});
...
router.post('/:id/respostas', requireAuth, validateRequest(createRespostaSchema), async (req, res) => {
    try {
        ...
        logger.error('Erro na rota POST /questoes-semanais/:id/respostas:', { error: error instanceof Error ? error.message : 'Erro desconhecido' });
        res.status(500).json({ error: 'Erro interno do servidor' });
    }
});
...
router.get('/:id/respostas', requireAuth, async (req, res) => {
    try {
      ...
      `, { count: 'exact' })
        ...
        logger.error('Erro na rota GET /questoes-semanais/:id/respostas:', { error: error instanceof Error ? error.message : 'Erro desconhecido' });
        res.status(500).json({ error: 'Erro interno do servidor' });
    }
});
...
router.get('/stats/ranking', requireAuth, async (req, res) => {
    try {
      ...
      `)
            ...
            .limit(Number(limit));
        if (error) {
            logger.error('Erro ao buscar ranking:', { error: error.message });
            res.status(500).json({ error: 'Erro interno do servidor' });
            return;
        }
        ...
        logger.error('Erro na rota GET /questoes-semanais/stats/ranking:', { error: error instanceof Error ? error.message : 'Erro desconhecido' });
        res.status(500).json({ error: 'Erro interno do servidor' });
    }
});
