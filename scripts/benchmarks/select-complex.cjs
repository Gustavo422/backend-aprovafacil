'use strict';
// Script de benchmark: SELECT complexo (JOIN, filtros, ORDER BY)
// Uso: node select-complex.cjs
...
const QUERY = process.env.BENCH_QUERY || `SELECT c.id, c.nome, cat.nome as categoria_nome FROM concursos c JOIN categorias_concursos cat ON c.categoria_id = cat.id WHERE c.ativo = true ORDER BY c.criado_em DESC LIMIT 10`;
...
async function main() {
  ...
  console.log('--- SELECT complexo benchmark ---');
