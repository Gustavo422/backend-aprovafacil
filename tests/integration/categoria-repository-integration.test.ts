 ...
 * Tests the integration between CategoriaRepository and SupabaseConnectionManager
 */
...
describe('Categoria Repository Integration Tests', () => {
  ...
  describe('Integração com SupabaseConnectionManager', () => {
    it('deve usar o cliente fornecido pelo SupabaseConnectionManager', () => {
      ...
      const clienteRepo = (repository as any).supabaseClient;
    ...
    it('deve atualizar o cliente quando o SupabaseConnectionManager é resetado', () => {
      ...
      const clienteAnterior = (repository as any).supabaseClient;
      
      // Criar um novo repositório que usa o cliente do connectionManager
      const novoRepo = new CategoriaRepository({
        supabaseClient: connectionManager.getClient()
      });
      ...
      const repoAposReset = new CategoriaRepository({
        supabaseClient: connectionManager.getClient()
      });
      
      // O cliente do novo repositório deve ser diferente do anterior
      expect((novoRepo as any).supabaseClient).toBe(clienteAnterior);
      expect((repoAposReset as any).supabaseClient).not.toBe(clienteAnterior);
    });
  });
});